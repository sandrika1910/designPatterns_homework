...
import Without_PageFactory.Pages.FirstMoviePage;
...
public class FirstMoviePageStep extends BasePage {
    WebDriverWait wait;
    public FirstMoviePageStep(WebDriver driver, int seconds) {
        super(driver);
    ...
    @Step
    public FirstMoviePageStep selectCaveEastPoint() {
        List<WebElement> places = firstMoviePage.getPlaces();
    ...
    @Step
    public FirstMoviePageStep checkReturnedOption() {
        WebElement parentElement = firstMoviePage.getCaveaEastPoint().findElement(By.xpath(".."));
        wait.until(ExpectedConditions.attributeToBe(parentElement,"aria-selected","true"));
        Assert.assertEquals(parentElement.getAttribute("aria-selected"), "true","'კავეა ისთ ფოინთი' is not selected");
        System.out.println("'კავეა ისთ ფოინთი' is selected");
        return this;
    }

    @Step
    public FirstMoviePageStep clickOnLast() {
        int windowHeight = driver.manage().window().getSize().getHeight();
        int elementPositionY = firstMoviePage.getLastOption().getLocation().getY();
        int scrollPosition = elementPositionY - (windowHeight / 2);
        wait.until(ExpectedConditions.elementToBeClickable(firstMoviePage.getLastDate()));
        actions.moveToElement(firstMoviePage.getLastDate()).click().build().perform();
        js.executeScript("window.scrollTo(0, arguments[0]);", scrollPosition);
        js.executeScript("arguments[0].click();", firstMoviePage.getLastOption());
//        actions.moveToElement(firstMoviePage.getLastOption()).click().build().perform();
        return this;
    }

    @Step
    public FirstMoviePageStep check() {
        String dateAttr = firstMoviePage.getLastDate().findElement(By.xpath("..")).getAttribute("aria-controls");
        String expectedCinemaName = firstMoviePage.getLastOption().findElement(By.xpath("./a/p[2]")).getText();
        String expectedMovieName = driver.findElement(By.xpath("//div[@class='movie-detailed-info']/div[2]/div/div/p")).getText();
        String expectedHour = firstMoviePage.getLastOption().findElement(By.xpath("./a/p")).getText();
        String expectedDay = firstMoviePage.getLastDate().getText().substring(0,2);
        ...
        String actualCinemaName = firstMoviePage.getActualCinemaNameElement().getText();
        String actualMovieName = firstMoviePage.getActualMovieNameElement().getText();
        String actualDateTime = firstMoviePage.getActualDateTimeElement().getText();
        wait.until(ExpectedConditions.attributeToBe(firstMoviePage.getDivElement(),"class","cinema-modal open"));
    ...
    @Step
    public FirstMoviePageStep chooseSeat() {
        WebElement freeSeat = firstMoviePage.getParentForSeats().findElement(By.xpath("./div[@class='seat free']"));
        freeSeat.click();
        return this;
    }
}
